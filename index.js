...
const fetching = async () => {
  ...
  $(".media-list__movie-info").each((i, el) => {
    listItems.push({
      ...
      actors: $(el)
        .find(".media-list__actors")
        .each((i, el) => el)
        .text(),
      consensus: $(el)
        .find(".media-list__consensus")
        .each((i, el) => el)
        .text(),
  ...
  db.serialize(() => {
    db.run(
      "CREATE TABLE films(title varchar(255), actors varchar(255), consensus varchar(255), date varchar(255))"
    );
    listItems.map((film) => {
      db.run(
        "INSERT INTO films (title, actors, consensus, date) VALUES (?, ?, ?, ?)",
        [film.title, film.actors, film.consensus, film.date],
        (err) => {
          if (err) {
            console.log(err);
          }
